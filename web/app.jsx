// AI Daily Digest — fun, girly-techy newsletter platform
...
// ─── DOODLES ───────────────────────────────────────────────────────────────
const Sparkle = ({ size = 18, color = "currentColor", style }) => (
...
const Squiggle = ({ width = 80, color = "currentColor", style }) => (
  <svg width={width} height="14" viewBox="0 0 80 14" style={style} aria-hidden fill="none">
...
// ─── HEADER ────────────────────────────────────────────────────────────────
const Header = ({ onHome, onArchive, route, currentDigest }) => (
  <header className="hdr">
    <button className="logo" onClick={onHome}>
      ...
      <span className="logo-text">
        <span className="logo-name">daily.exe</span>
...
const CategoryCard = ({ cat, index, onOpen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="cat-emoji" aria-hidden>{cat.emoji}</div>
      ...
      <div className="cat-tags">
        {cat.stories.slice(0, 3).map((s, i) => (
          <span key={i} className="cat-tag">{s.tags[0]}</span>
        ))}
      </div>
      <div className="cat-glyph" aria-hidden style={{ color: accent.deep }}>{accent.glyph}</div>
    </button>
  );
};

const CategoryGrid = ({ digest, onOpen }) => (
  <section className="cat-grid-wrap">
    <div className="cat-grid-head">
      <h2 className="section-title">
        Pick your <em className="hl-lilac">flavor</em> of news <Sparkle size={22} color="var(--accent-pink-deep)" style={{ display: "inline", verticalAlign: "middle", marginLeft: 6 }} />
      </h2>
      ...
      <button className="story-head" onClick={() => setOpen(!open)}>
        ...
        <div className="story-head-main">
          ...
          <div className="story-meta">
            ...
            {story.tags.map((t, i) => (<span key={i} className="story-tag">#{t}</span>))}
          </div>
        </div>
        <span className="story-toggle" aria-hidden>
  ...
  return (
    <div className="digest" style={{ "--cat-bg": a.bg, "--cat-deep": a.deep, "--cat-soft": a.soft, "--cat-ink": a.ink }}>
      ...
      <header className="digest-hero">
        <div className="digest-hero-emoji" aria-hidden>{cat.emoji}</div>
      ...
      <section className="stories">
        {cat.stories.map((s, i) => (<StoryCard key={i} story={s} accent={cat.accent} index={i} />))}
      </section>
    ...
    <button className="arch-card" onClick={() => onOpen(digest.iso)} style={{ animationDelay: `${index * 40}ms` }}>
      ...
      <div className="arch-card-mid">
        <div className="arch-meta">
          ...
          {isToday && <span className="arch-today-pill">today</span>}
        </div>
      ...
      <div className="arch-card-arrow" aria-hidden>
      ...
      <header className="arch-hero">
        <div className="arch-hero-text">
          ...
          <p className="arch-hero-sub">
            {DIGESTS.length} issues · {DIGESTS.reduce((n, d) => n + d.categories.reduce((m, c) => m + c.stories.length, 0), 0)} stories. Click any day to revisit.
          </p>
        </div>
      ...
      <div className="arch-list">
        {filtered.length === 0 ? (
          <div className="arch-empty">
            ...
            <p>no issues match "{query}". try a different keyword?</p>
          </div>
        ) : (
          filtered.map((d, i) => (
            <ArchiveCard key={d.iso} digest={d} onOpen={onOpen} isToday={i === 0 && !query} index={i} />
          ))
        )}
      </div>
    </div>
      ...
      <div className="subscribe-inner">
        ...
        <form className="subscribe-form" onSubmit={handleSubmit}>
          {status === "idle" || status === "loading" ? (
            <>
              <input type="email" required placeholder="your@email" value={email} onChange={e => setEmail(e.target.value)} disabled={status === "loading"} />
              <button type="submit" disabled={status === "loading"}>
                {status === "loading" ? "…" : <>subscribe<Sparkle size={14} color="currentColor" style={{ marginLeft: 8, verticalAlign: "middle" }} /></>}
              </button>
            </>
          ) : status === "done" ? (
...
const Footer = () => (
  <footer className="ftr">
    <div className="ftr-row">
      <span>daily.exe — built with curiosity</span>
...
// ─── HOME (landing for a given digest) ─────────────────────────────────────
const Home = ({ digest, isHistorical, onOpen, onArchive }) => (
  <>
...
const PALETTES = {
  candy: {
    bg: "#FBF7F4", ink: "#1A1613",
    pink: { bg: "#FFD6E7", deep: "#E8369A", ink: "#5A0F36", soft: "#FFEEF5" },
    lilac: { bg: "#E4D9FF", deep: "#7C4DFF", ink: "#2F1A6E", soft: "#F2EDFF" },
    lime: { bg: "#DEFF8C", deep: "#6B9E00", ink: "#2A3A00", soft: "#F2FFD6" },
  },
  cyber: {
    bg: "#0E0B14", ink: "#F4ECFF",
    pink: { bg: "#FF5FB1", deep: "#FF8FD0", ink: "#FFE4F2", soft: "#3A1126" },
    lilac: { bg: "#9B7BFF", deep: "#C7B6FF", ink: "#EDE5FF", soft: "#1F1638" },
    lime: { bg: "#C8FF3D", deep: "#E2FF7A", ink: "#1A2200", soft: "#1F2A05" },
  },
  pastel: {
    bg: "#FFF9F2", ink: "#2A1F2E",
    pink: { bg: "#FFE0EC", deep: "#D9568D", ink: "#5A0F36", soft: "#FFF1F6" },
    lilac: { bg: "#E8E0FF", deep: "#8B6BD9", ink: "#2F1A6E", soft: "#F4F0FF" },
    lime: { bg: "#E8F4C0", deep: "#88A93E", ink: "#2A3A00", soft: "#F4FAE0" },
  },
  espresso: {
    bg: "#F5EDE2", ink: "#2A1A0E",
    pink: { bg: "#F4B8C4", deep: "#C9416A", ink: "#4F0F22", soft: "#FBE4EA" },
    lilac: { bg: "#D4C4E4", deep: "#7553A8", ink: "#2A1648", soft: "#EDE4F4" },
    lime: { bg: "#D4DC8C", deep: "#6B7A1A", ink: "#1F2800", soft: "#EAF0C8" },
  },
};
...
function App() {
  // route: { view: 'home' | 'archive' | 'digest', iso: string, catId: string|null }
  ...
  return (
    <div className="app" style={cssVars} data-sparkles={tweaks.showSparkles ? "on" : "off"} data-screen-label={screenLabel}>
      ...
      <main className="main">
        {route.view === "archive" && <ArchiveView onOpen={openDigestDay} />}
        {route.view === "home" && <Home digest={currentDigest} isHistorical={isHistorical} onOpen={openCategory} onArchive={goArchive} />}
        {route.view === "digest" && <DigestView digest={currentDigest} catId={route.catId} onBackHome={backToDayHome} onPickCat={pickCat} />}
      </main>
